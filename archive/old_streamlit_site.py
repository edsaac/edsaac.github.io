...
def generate_badge_table(badges: list):
    ...
    st.write(f'<p align="center">{chain_badges}</p>', unsafe_allow_html=True)
...
def generate_badge(kind: str, link: Union[str, bool]) -> Union[str, None]:
    if link:
        if kind == "Streamlit App":
            img_src = "https://static.streamlit.io/badges/streamlit_badge_black_white.svg"
...
def main():
    ...
    with cols[1]:
        ...
        if category == "Conferences":
            ...
            class_code = st.selectbox("Select a course:", options=database["Classes & Labs"].keys(), index=1)
